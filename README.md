...
jobs:
  claude-response:
    ...
    steps:
      - uses: LRTmHBFt6-xwxrwx/claude-code-action@main  # Fork with OAuth support
        with:
...
steps:
  - uses: LRTmHBFt6-xwxrwx/claude-code-action@main
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`LRTmHBFt6-xwxrwx/claude-code-base-action`](https://github.com/LRTmHBFt6-xwxrwx/claude-code-base-action).
